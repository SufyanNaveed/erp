...
class Report_model extends CI_Model {
 ...
 public function bdtask_getStock($postData=null){
        ...
        $this->db->group_by('a.product_id');
         ...
         $searchQuery .= "or m.supplier_name like '%".$searchValue."%') ";
         ## Fetch records
         $this->db->select("a.*,
                a.product_name,
                a.product_id,
        ...
        foreach($records as $record ){
            ...
            $stockout2 = $this->db->select('sum(quantity) as totalDeliveryQnty,Avg(rate) as deliveryprice')->from('delivery_note_details')->where('product_id',$record->product_id)->get()->row();
            //$stockout =  $stockout1 +  $stockout2;
