...
public function credit_voucher_receipt($voucher_no){
  $voucher_no           = $this->uri->segment(2);

  $data['account_info']  = $this->account_model->accountinfo($voucher_no);
  $data['voucher_info']  = $this->account_model->voucherinfo($voucher_no);
  $data['title']         = display('credit_voucher_receipt');
  $data['module']        = "account";
  $data['page']          = "credit_voucher_receipt"; 
  echo modules::run('template/layout', $data);
}
public function debit_voucher_receipt($voucher_no){
  $voucher_no           = $this->uri->segment(2);

  $data['account_info']  = $this->account_model->accountinfo2($voucher_no);
  $data['voucher_info']  = $this->account_model->voucherinfo2($voucher_no);
  $data['title']         = display('debit_voucher_receipt');
  $data['module']        = "account";
  $data['page']          = "debit_voucher_receipt"; 
  echo modules::run('template/layout', $data);
}
    // Contra Voucher form
