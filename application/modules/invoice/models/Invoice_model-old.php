     ...
     public function getInvoiceList($postData=null){
         ...
         foreach($records as $record ){
          ...
          $jsaction = "return confirm('Are You Sure ?')";
           ...
           $button .='  <a href="'.$base_url.'invoice_details/'.$record->invoice_id.'" class="btn print_invoice btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('invoice').'"><i class="fal fa-window-restore" aria-hidden="true"></i></a>';

         $button .='  <a href="'.$base_url.'invoice_pad_print/'.$record->invoice_id.'" class="btn action_btn btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('pad_print').'"><i class="fal fa-fax" aria-hidden="true"></i></a>';

         $button .='  <a href="'.$base_url.'pos_print/'.$record->invoice_id.'" class="btn payment_btn" data-toggle="tooltip" data-placement="left" title="'.display('pos_invoice').'"><i class="fal fa-fax" aria-hidden="true"></i></a>';
          $button .='  <a href="'.$base_url.'download_invoice/'.$record->invoice_id.'" class="btn details_btn" data-toggle="tooltip" data-placement="left" title="'.display('download').'"><i class="fal fa-download"></i></a>';

      if($this->permission1->method('manage_invoice','update')->access()){
         $button .=' <a href="'.$base_url.'invoice_edit/'.$record->invoice_id.'" class="btn edit_btn" data-toggle="tooltip" data-placement="left" title="'. display('update').'"><i class="fal fa-pencil" aria-hidden="true"></i></a> ';
     }
    
      if($this->permission1->method('manage_invoice','delete')->access()){
         $button .=' <a href="'.$base_url.'invoice_delete/'.$record->invoice_id.'" class="btn delete_btn" data-toggle="tooltip" data-placement="left" title="'. display('delete').'"><i class="fal fa-trash-alt" aria-hidden="true"></i></a> ';
     }
