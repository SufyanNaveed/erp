//Add Input Field Of Row
    "use strict";
function addInputField_delivery_note(t) {
    var sel = $("#invoicetype").val();
    var row = $("#normaldelivery_note tbody tr").length;
    var count = row + 1;
      var  tab1 = 0;
      var  tab2 = 0;
      var  tab3 = 0;
    ...
    if (count == limits)
        ...
        e.innerHTML = "<td><input type='text' name='product_name' onkeypress='delivery_note_productList(" + count + ");' class='form-control productSelection common_product' placeholder='Product Name' id='" + a + "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_value  product_id_" + count + "' name='product_id[]' id='SchoolHiddenId'/></td><td style='display:none'><input type='text' name='desc[]'' class='form-control text-right ' tabindex='" + tab2 + "'/></td><td style='display:none'><select class='form-control' id='serial_no_" + count + "' name='serial_no[]' tabindex='" + tab3 + "'><option></option></select></td><td class='cls1'>    <input type='text' name='sales_unit[]' tabindex='"+tab2+"'  id='sales_unit_"+ count +"' class='form-control text-right' onkeyup='' onchange='' placeholder='0.00' value='' min='0' readonly/><input type='hidden' name='item_pcs[]' id='item_pcs_" + count + "' value=''/></td><td class='text-right'><input type='text' name='unit[]' tabindex='"+tab7+"'  id='unit_"+ count +"' class='form-control text-right' onkeyup='' onchange='' value='' readonly/></td><td class='text-right cls1'><input type='text' name='Cartons[]' tabindex='"+tab2+"'  id='Cartons_"+ count +"' class='form-control text-right' onkeyup='CheckCartons2("+ count +");' onchange='CheckCartons2("+ count +");' placeholder='0.00' value='' /></td><td class='text-right cls1'><input type='text' name='pcs[]' tabindex='"+tab7+"' id='pcs_"+ count +"' class='form-control text-right' onkeyup='CheckCartons2("+ count +");' onchange='CheckCartons2("+ count +");' ></td><td class='text-right cls1'><input type='text' name='tot_ctn[]' tabindex='"+tab7+"'  id='tot_ctn_"+ count +"' class='form-control text-right' onkeyup='CheckCartons2("+ count +");' onchange='CheckCartons2("+ count +");' placeholder='0.00' value='' /></td> <td><input type='text' name='available_quantity[]' id='' class='form-control text-right common_avail_qnt available_quantity_" + count + "' value='0' readonly='readonly' /></td><td><input class='form-control text-right common_name unit_" + count + " valid' value='None' readonly='' aria-invalid='false' type='text'></td><td> <input type='text' name='product_quantity[]' value='1' required='required' onkeyup='bdtask_delivery_note_quantity_calculate(" + count + ");' onchange='bdtask_delivery_note_quantity_calculate(" + count + ");' id='total_qntt_" + count + "' class='common_qnt total_qntt_" + count + " form-control text-right'  placeholder='0.00' min='0' tabindex='" + tab3 + "'/></td><td><input type='text' name='product_rate[]' onkeyup='bdtask_delivery_note_quantity_calculate(" + count + ");' onchange='bdtask_delivery_note_quantity_calculate(" + count + ");' id='price_item_" + count + "' class='common_rate price_item" + count + " form-control text-right' required placeholder='0.00' min='0' tabindex='" + tab4 + "'/></td><td><input type='text' name='discount[]' onkeyup='bdtask_delivery_note_quantity_calculate(" + count + ");' onchange='bdtask_delivery_note_quantity_calculate(" + count + ");' id='discount_" + count + "' class='form-control text-right common_discount' placeholder='0.00' min='0' tabindex='" + tab5 + "' /><input type='hidden' value='' name='discount_type' id='discount_type_" + count + "'></td><td class='text-right'><input class='common_total_price total_price form-control text-right' type='text' name='total_price[]' id='total_price_" + count + "' value='0.00' readonly='readonly'/></td><td>"+tbfild+"<input type='hidden' id='all_discount_" + count + "' class='total_discount dppr' name='discount_amount[]'/><button tabindex='" + tab5 + "' style='text-align: right;' class='btn delete_btn' type='button' value='Delete' onclick='deleteRow_delivery_note(this)'><i class='fal fa-trash-alt'></i></button></td>",
                ...
                document.getElementById("delivery_note_discount").setAttribute("tabindex", tab8);
                ...
                document.getElementById("add_delivery_note").setAttribute("tabindex", tab12);
                count++
    }
}
//Quantity calculat
    "use strict";
function bdtask_delivery_note_quantity_calculate(item) {
    var quantity = $("#total_qntt_" + item).val();
    var available_quantity = $(".available_quantity_" + item).val();
    var price_item = $("#price_item_" + item).val();
    var delivery_note_discount = $("#delivery_note_discount").val();
...
//Calculate Sum
    "use strict";
function delivery_note_calculateSum() {
     var taxnumber = $("#txfieldnum").val();
      var t = 0,
            a = 0,
            e = 0,
...
var j = 0;
 ...
 $(".dppr").each(function () {
    ...
    var invdis = $("#delivery_note_discount").val();
...
//Invoice Paid Amount
    "use strict";
function delivery_note_paidamount() {
...
function stockLimit(t) {
    ...
    $.ajax({
        type: "POST",
        url: o + "Cdelivery_note/product_stock_check",
        data: {
            product_id: e
        },
        cache: !1,
...
//Invoice full paid
    "use strict";
function delivery_notee_full_paid() {
...
//Delete a row of table
    "use strict";
function deleteRow_delivery_note(t) {
    var a = $("#normaldelivery_note > tbody > tr").length;
    if (1 == a)
        ...
        delivery_note_paidamount();
        var current = 1;
        $("#normaldelivery_note > tbody > tr td input.productSelection").each(function () {
            current++;
            $(this).attr('id', 'product_name' + current);
        });
        var common_qnt = 1;
        $("#normaldelivery_note > tbody > tr td input.common_qnt").each(function () {
            common_qnt++;
            $(this).attr('id', 'total_qntt_' + common_qnt);
            $(this).attr('onkeyup', 'bdtask_delivery_note_quantity_calculate('+common_qnt+');');
            $(this).attr('onchange', 'bdtask_delivery_note_quantity_calculate('+common_qnt+');');
        });
        var common_rate = 1;
        $("#normaldelivery_note > tbody > tr td input.common_rate").each(function () {
            common_rate++;
        ...
        $("#normaldelivery_note > tbody > tr td input.common_discount").each(function () {
            common_discount++;
        ...
        $("#normaldelivery_note > tbody > tr td input.common_total_price").each(function () {
            common_total_price++;
...
var count = 2,
   ...
   var style ='none';
    ...
    $('#normaldelivery_note .toggle').on('click', function() {
    $('#normaldelivery_note .hideableRow').toggleClass('hiddenRow');
  })
});
   ...
   var base_url = $("#base_url").val();
        $.ajax({
            url: base_url + 'delivery_note/delivery_note/previous',
            type: 'post',
    ...
    var options = {
        ...
        $.ajax( {
          url: base_url + "delivery_note/delivery_note/bdtask_customer_autocomplete",
          method: 'post',
          dataType: "json",
          data: {
            term: request.term,
...
$(document).ready(function(){
    ...
    $('#full_paid_tab').keydown(function(event) {
        if(event.keyCode == 13) {
 $('#add_delivery_note').trigger('click');
        }
    });
});
...
//       }, function(inputValue) {
//           if (inputValue===true) {
//       $("#normaldelivery_note tbody tr").remove();
...
//       });
$("#inv_id").val(data.delivery_note_id);
$('#printconfirmodal').modal('show');
   ...
   var frm = $("#update_delivery_note");
    ...
    "use strict";
     function delivery_note_productList(sl) {
    ...
    var options = {
        ...
        $.ajax( {
          url: base_url + "delivery_note/delivery_note/bdtask_autocomplete_product",
          method: 'post',
          dataType: "json",
          data: {
            term: request.term,
       ...
       select: function( event, ui ) {
            $(this).parent().parent().find(".autocomplete_hidden_value").val(ui.item.value); 
                ...
                $.ajax
                   ({
                        type: "POST",
                        url: base_url+"delivery_note/delivery_note/retrieve_product_data_inv",
                        ...
                        success: function(data)
                        {
                           ...
                           $('.'+txclass).val(obj.taxdta[i]);
                            ...
                            $('#'+serial_no).html(obj.serial);
                                   ...
                                   bdtask_delivery_note_quantity_calculate(sl);
