...
<span class="top-fixed-link">
      <?php
           ...
           ?>
           <a href="<?php echo base_url('add_invoice')?>" class="btn btn-primary btn-outline"><i class="fa fa-balance-scale"></i> <?php  echo display('invoice') ?></a>
     <?php }?>

     
        <?php if($this->permission1->method('customer_receive','create')->access()){ ?>
           <a href="<?php echo base_url('customer_receive')?>" class="btn btn-primary btn-outline"><i class="fa fa-money"></i> <?php echo display('customer_receive')?></a>
       <?php } ?>
      
  <?php if($this->permission1->method('supplier_payment','create')->access()){ ?>
          <a href="<?php echo base_url('supplier_payment')?>" class="btn btn-primary btn-outline"><i class="fa fa-money" aria-hidden="true"></i> <?php echo display('supplier_payment')?></a>
      <?php } ?>

<?php if($this->permission1->method('add_purchase','create')->access()){ ?>
          <a href="<?php echo base_url('add_purchase')?>" class="btn btn-primary btn-outline"><i class="ti-shopping-cart"></i> <?php echo display('purchase') ?></a>
 <?php } ?>
</span>
    ...
    <div class="navbar-custom-menu">
        ...
        <ul class="nav navbar-nav">
           ...
           ?>
             <li>
                    <a href="<?php echo base_url('gui_pos')?>" class="text-white  btn-primary " style="height: 33px; padding: 4px 3px;"> <span class="fa fa-plus"></span> <?php echo display('pos_invoice') ?></a>
                </li>
              <?php }?>
