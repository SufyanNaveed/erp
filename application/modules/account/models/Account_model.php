...
public function accountinfo($voucher_no){
  $result =  $this->db->select('*')
     ...
     $result2 = $this->db->select('*')->from('acc_coa')->where('HeadCode',$result[0]['COAID'])->get()->row();
                 
      return $result2->HeadName;
               

}
public function voucherinfo2($voucher_no){
  return  $result =  $this->db->select('*')
                  ->from('acc_transaction')
                  ->where('VNo',$voucher_no)
                  ->get()
...
public function accountinfo2($voucher_no){
  $result =  $this->db->select('*')
                  ->from('acc_transaction')
                  ->where('VNo',$voucher_no)
                  ->where('VNo',$voucher_no)
 ...
 return true;
    ...
    public function Transacc()
    {
      return  $data = $this->db->select("*")
            ->from('acc_coa')
            ->join('supplier_information m','m.supplier_id = acc_coa.supplier_id')
            ->where('acc_coa.supplier_id !=', NULL)  
            ->where('IsTransaction', 1)  
            ->where('IsActive', 1) 
            ->order_by('acc_coa.HeadName')
            ->get()
            ->result();
    }
    ...
    $this->session->set_flashdata('message', display('save_successfully'));
    redirect('debit_voucher_receipt/'.$voucher_no);
    //return true;
}
    ...
    public function insert_creditvoucher(){
          ...
          $this->session->set_flashdata('message', display('save_successfully'));
          redirect('credit_voucher_receipt/'.$voucher_no);
   // return true;
}
