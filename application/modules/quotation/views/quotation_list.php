                ...
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body">
                        <div class="table-responsive" id="results">
                            <table id="dataTableExample2" class="table table-striped table-hover">
                                <thead>
                                    <tr>
                        ...
                        foreach ($quotation_list as $quotation) {
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                        ...
                                        <a href="<?php echo base_url() . 'quotation_details/' . $quotation->quotation_id; ?>" class="btn details_btn"   title="<?php echo display('details') ?>" data-original-title="<?php echo display('details') ?> "><i class="fal fa-eye" aria-hidden="true"></i></a>
                                       <?php
                                    if ($quotation->status == 1) {?>
                                         <?php if($this->permission1->method('manage_quotation','update')->access()){ ?>
                                        <a href="<?php echo base_url() . 'edit_quotation/' . $quotation->quotation_id; ?>" class="btn edit_btn" title="<?php echo display('update') ?>" data-original-title="<?php echo display('update') ?> "><i class="fal fa-edit" aria-hidden="true"></i></a>
                                    <?php }?>
                                      <?php }?>
                                       
                                        <?php if($this->permission1->method('manage_quotation','delete')->access()){ ?>
                                        <a href="<?php echo base_url() . 'quotation/quotation/delete_quotation/' . $quotation->quotation_id; ?>" class="btn delete_btn"  onclick="return confirm('Are You Sure To Want to Delete ??')" title="<?php echo display('delete') ?>" data-original-title="<?php echo display('delete') ?> "><i class="fal fa-trash-alt" aria-hidden="true"></i></a>
                                    <?php }?>
                           
                                    <a href="<?php echo base_url('quotation/quotation/quotation_download/' . $quotation->quotation_id); ?>" class="btn payment_btn"  title="<?php echo display('download') ?>" data-original-title="<?php echo display('download') ?> "><i class="fal fa-download" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                            <?php
