...
defined('BASEPATH') OR exit('No direct script access allowed');
 #------------------------------------    
    # Author: Monedge Solutions Pvt. Ltd
 ...
 public function supplier_list(){
     ...
     public function product_search_item($supplier_id, $product_name) {
      $query=$this->db->select('*')
                ...
                // ->where('a.supplier_id',$supplier_id) 10-1-23
        ...
        $data2 = array(
            ...
            'sales_unit'     => $product_information->salesunit,
            'pcsperboxid'     => $product_information->pcsperboxid,
        );
        ...
        $this->db->join('supplier_information b', 'b.supplier_id = a.supplier_id','left');
         ...
         foreach($records as $record ){
          ...
          $jsaction = "return confirm('Are You Sure ?')";
           ...
           $button .='  <a href="'.$base_url.'purchase_details/'.$record->purchase_id.'" class="btn details_btn" data-toggle="tooltip" data-placement="left" title="'.display('purchase_details').'"><i class="fal fa-window-restore" aria-hidden="true"></i></a>';
      if($this->permission1->method('manage_purchase','update')->access()){
         $button .=' <a href="'.$base_url.'purchase_edit/'.$record->purchase_id.'" class="btn edit_btn" data-toggle="tooltip" data-placement="left" title="'. display('update').'"><i class="fal fa-pencil" aria-hidden="true"></i></a> ';
     }
      if($this->permission1->method('manage_purchase','delete')->access()){
         $button .=' <a href="'.$base_url.'purchase_delete/'.$record->purchase_id.'" class="btn delete_btn" data-toggle="tooltip" data-placement="left" title="'. display('delete').'"><i class="fal fa-trash-alt" aria-hidden="true"></i></a> ';
     }
