...
class Delivery_note extends MX_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'delivery_note_model','customer/customer_model')); 
    ...
    function bdtask_delivery_note_form() {
        $walking_customer      = $this->delivery_note_model->pos_customer_setup();
        $data['customer_name'] = $walking_customer[0]['customer_name'];
        $data['customer_id']   = $walking_customer[0]['customer_id'];
        $data['delivery_note_no']    = $this->number_generator();
        $data['title']         = display('add_delivery_note');
        $data['taxes']         = $this->delivery_note_model->tax_fileds();
        $data['module']        = "delivery_note";
        $data['page']          = "add_delivery_note_form"; 
        echo modules::run('template/layout', $data);
    }

    public function bdtask_delivery_note_list(){
        $data['title']        = display('manage_delivery_note');
        $data['total_delivery_note']= $this->delivery_note_model->count_delivery_note();
        $data['module']       = "delivery_note";
        $data['page']         = "delivery_note"; 
        echo modules::run('template/layout', $data);
    }

     public function CheckDeliveryNoteList(){
       
        $postData = $this->input->post();
        $data     = $this->delivery_note_model->getDeliveryNoteList($postData);
    ...
    public function bdtask_delivery_note_details($delivery_note_id = null){
        ...
        if (!empty($delivery_note_detail)) {
            foreach ($delivery_note_detail as $k => $v) {
                $delivery_note_detail[$k]['final_date'] = $delivery_note_detail[$k]['date'];
        ...
        $users         = $this->delivery_note_model->user_delivery_note_data($user_id);
        $data = array(
        'title'             => display('delivery_note_details'),
        ...
        'total_amount'      => number_format($delivery_note_detail[0]['total_amount']+$delivery_note_detail[0]['prevous_due'], 2, '.', ','),
        ...
        'is_discount'       => $delivery_note_detail[0]['total_discount']-$delivery_note_detail[0]['delivery_note_discount'],
        ...
        $data['module']     = "delivery_note";
        $data['page']       = "delivery_note_html"; 
        echo modules::run('template/layout', $data);
    }


    public function bdtask_delivery_note_pad_print($delivery_note_id){
           $delivery_note_detail = $this->delivery_note_model->retrieve_delivery_note_html_data($delivery_note_id);
         $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
        ...
        'customer_email'   => $delivery_note_detail[0]['customer_email'],
        'final_date'       => $delivery_note_detail[0]['final_date'],
        'print_setting'    => $this->delivery_note_model->bdtask_print_settingdata(),
        'delivery_note_details'  => $delivery_note_detail[0]['delivery_note_details'],
    ...
    public function bdtask_delivery_note_pos_print($delivery_note_id = null){
        $delivery_note_detail = $this->delivery_note_model->retrieve_delivery_note_html_data($delivery_note_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
     ...
     public function bdtask_pos_print_direct(){
        ...
        $data['module']     = "delivery_note";
        $data['page']       = "pos_delivery_note_html_direct"; 
        echo modules::run('template/layout', $data);

    }


    public function bdtask_download_delivery_note($delivery_note_id = null){
        $delivery_note_detail = $this->delivery_note_model->retrieve_delivery_note_html_data($delivery_note_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
        ...
        $currency_details = $this->delivery_note_model->retrieve_setting_editdata();
        $company_info     = $this->delivery_note_model->retrieve_company();
        $totalbal         = $delivery_note_detail[0]['total_amount']+$delivery_note_detail[0]['prevous_due'];
        $amount_inword    = $this->numbertowords->convert_number($totalbal);
        $user_id          = $delivery_note_detail[0]['sales_by'];
        $users            = $this->delivery_note_model->user_delivery_note_data($user_id);
        ...
        'title'             => display('delivery_note_details'),
        'delivery_note_id'        => $delivery_note_detail[0]['delivery_note_id'],
        'customer_info'     => $delivery_note_detail,
        'delivery_note_no'        => $delivery_note_detail[0]['delivery_note'],
        'customer_name'     => $delivery_note_detail[0]['customer_name'],
        'customer_address'  => $delivery_note_detail[0]['customer_address'],
        'customer_mobile'   => $delivery_note_detail[0]['customer_mobile'],
        ...
        $dompdf = new DOMPDF();
        $page = $this->load->view('delivery_note/delivery_note_download', $data, true);
        $file_name = time();
        $dompdf->load_html($page,'UTF-8');
        $dompdf->render();
        $output = $dompdf->output();
        file_put_contents("assets/data/pdf/delivery_note/$file_name.pdf", $output);
        $filename = $file_name . '.pdf';
        $file_path = base_url() . 'assets/data/pdf/delivery_note/' . $filename;

        $this->load->helper('download');
        force_download('./assets/data/pdf/delivery_note/' . $filename, NULL);
        redirect("delivery_note_list");
    }

    public function bdtask_manual_delivery_note_insert(){
    ...
    $this->form_validation->set_rules('delivery_note_no', display('delivery_note_no') ,'required|max_length[20]|is_unique[delivery_note.delivery_note]');
 ...
 //die($this->form_validation->run());
        if ($this->form_validation->run() == 1) {
         // if ($this->form_validation->run() === true) {  by ammar 19-7-22

        $delivery_note_id = $this->delivery_note_model->delivery_note_entry();
       // die($delivery_note_id);
        ...
        if($mailsetting[0]['isdelivery_note']==1){
        $mail = $this->delivery_note_pdf_generate($delivery_note_id);
        if($mail == 0){
        $data['exception'] = $this->session->set_userdata(array('error_message' => display('please_config_your_mail_setting')));
          }
        }
        if($normal == 1){
        $printdata = $this->bdtask_delivery_note_details_directprint($delivery_note_id);
        $data['details'] = $this->load->view('delivery_note/delivery_note_html_manual', $printdata, true);
        }else{
        $printdata = $this->delivery_note_model->bdtask_delivery_note_pos_print_direct($delivery_note_id);
        $data['details'] = $this->load->view('delivery_note/pos_print', $printdata, true);
        }
    ...
    public function bdtask_edit_delivery_note($delivery_note_id = null){
        $delivery_note_detail = $this->delivery_note_model->retrieve_delivery_note_editdata($delivery_note_id);
        $taxinfo        = $this->delivery_note_model->delivery_note_taxinfo($delivery_note_id);
        ...
        if (!empty($delivery_note_detail)) {
            foreach ($delivery_note_detail as $k => $v) {
                $i++;
                $delivery_note_detail[$k]['sl'] = $i;
                $stock = $this->delivery_note_model->stock_qty_check($delivery_note_detail[$k]['product_id']);
                $delivery_note_detail[$k]['stock_qty'] = $stock + $delivery_note_detail[$k]['quantity'];
            }
        }

        $currency_details = $this->delivery_note_model->retrieve_setting_editdata();
        $data = array(
            'title'           => display('delivery_note_edit'),
            'delivery_note_id'      => $delivery_note_detail[0]['delivery_note_id'],
            'customer_id'     => $delivery_note_detail[0]['customer_id'],
            'customer_name'   => $delivery_note_detail[0]['customer_name'],
            'date'            => $delivery_note_detail[0]['date'],
            'delivery_note_details' => $delivery_note_detail[0]['delivery_note_details'],
            'delivery_note'         => $delivery_note_detail[0]['delivery_note'],
            'total_amount'    => $delivery_note_detail[0]['total_amount'],
            'paid_amount'     => $delivery_note_detail[0]['paid_amount'],
            'due_amount'      => $delivery_note_detail[0]['due_amount'],
            'delivery_note_discount'=> $delivery_note_detail[0]['delivery_note_discount'],
            'total_discount'  => $delivery_note_detail[0]['total_discount'],
            'unit'            => $delivery_note_detail[0]['unit'],
            'tax'             => $delivery_note_detail[0]['tax'],
             'taxes'          => $taxfield,
            'prev_due'        => $delivery_note_detail[0]['prevous_due'],
            'net_total'       => $delivery_note_detail[0]['prevous_due'] + $delivery_note_detail[0]['total_amount'], 
            'shipping_cost'   => $delivery_note_detail[0]['shipping_cost'],
            'total_tax'       => $delivery_note_detail[0]['taxs'],
            'delivery_note_all_data'=> $delivery_note_detail,
            'taxvalu'         => $taxinfo,
            'discount_type'   => $currency_details[0]['discount_type'],
            'bank_id'         => $delivery_note_detail[0]['bank_id'],
            'paytype'         => $delivery_note_detail[0]['payment_type'],
        );
        $data['module']     = "delivery_note";
        $data['page']       = "edit_delivery_note_form"; 
        echo modules::run('template/layout', $data);
    }

    public function bdtask_update_delivery_note(){
     $this->form_validation->set_rules('customer_id', display('customer_name') ,'required|max_length[15]');
    $this->form_validation->set_rules('paytype', display('payment_type') ,'required|max_length[20]');
    $this->form_validation->set_rules('delivery_note_no', display('delivery_note_no') ,'required|max_length[20]');
    $this->form_validation->set_rules('product_id[]',display('product'),'required|max_length[20]');
    $this->form_validation->set_rules('product_quantity[]',display('quantity'),'required|max_length[20]');
    $this->form_validation->set_rules('product_rate[]',display('rate'),'required|max_length[20]');

    if ($this->form_validation->run() === true) {
       $delivery_note_id = $this->delivery_note_model->update_delivery_note();
        if(!empty($delivery_note_id)){
        $data['status'] = true;
        $data['delivery_note_id'] = $delivery_note_id;
        ...
        $data['details'] = $this->load->view('delivery_note/delivery_note_html', $data, true);
        }else{
        $data['status'] = false;
        $data['exception'] = 'Please Try Again';
        }
     ...
     echo json_encode($data);
        ...
        function bdtask_pos_delivery_note() {
        $taxfield = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
                ->get()
                ->result_array();
        ...
        $data['customer_id']   = $walking_customer[0]['customer_id'];
        $data['delivery_note_no']    = $this->number_generator();
        $data['title']         = display('pos_delivery_note');
        $data['taxes']         = $this->delivery_note_model->tax_fileds();
        $data['taxnumber']     = $num_column;
        $data['module']        = "delivery_note";
        $data['page']          = "add_pos_delivery_note_form"; 
        echo modules::run('template/layout', $data);
    }
  ...
  public function bdtask_gui_pos(){
            ...
            $walking_customer      = $this->delivery_note_model->walking_customer();
            $data['customer_id']   = $walking_customer[0]['customer_id'];
            $data['customer_name'] = $walking_customer[0]['customer_name'];
            $data['categorylist']  = $this->delivery_note_model->category_list();
            $customer_details      = $this->delivery_note_model->pos_customer_setup();
            $data['customerlist']  = $this->delivery_note_model->customer_dropdown();
            $data['customer_name'] = $customer_details[0]['customer_name'];
            $data['customer_id']   = $customer_details[0]['customer_id'];
            $data['itemlist']      = $this->delivery_note_model->allproduct();
            $data['product_list']  = $this->delivery_note_model->product_list();
            $data['taxes']         = $taxfield;
            $data['taxnumber']     = $num_column;
            $data['delivery_note_no']    = $this->number_generator();
            $data['todays_delivery_note']= $this->delivery_note_model->todays_delivery_note();
            $data['module']        = "delivery_note";
            $data['page']          = "gui_pos_delivery_note"; 
            echo modules::run('template/layout', $data); 
        }
      ...
      public function getitemlist(){
                ...
                $getproduct  = $this->delivery_note_model->searchprod($category_id);
                if(!empty($getproduct)){
                $data['itemlist']=$getproduct;
                $this->load->view('delivery_note/getproductlist', $data);  
                }else{
                $title['title'] = 'Product Not found';
                $this->load->view('delivery_note/productnot_found', $title);
                }
        }
     ...
     public function getitemlist_byname(){
            ...
            $getproduct       = $this->delivery_note_model->searchprod_byname($product_name);
            if(!empty($getproduct)){
            $data['itemlist'] = $getproduct;
            $this->load->view('delivery_note/getproductlist', $data);  
            }else{
        ...
        public function getitemlist_byproductname(){
                ...
                $catid      = $this->input->post('category_id',TRUE);
                $getproduct = $this->delivery_note_model->searchprod_byname($catid,$prod);
                if(!empty($getproduct)){
                $data['itemlist']=$getproduct;
                $this->load->view('delivery_note/getproductlist', $data);  
                }
         ...
         public function gui_pos_delivery_note() {
        $product_id = $this->input->post('product_id',TRUE);
        $product_details = $this->delivery_note_model->pos_delivery_note_setup($product_id);
        $taxfield       = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
                ->get()
                ->result_array();
        ...
        if (empty($serialdata)) {
            ...
            $tr .= "<tr id=\"row_" . $product_details->product_id . "\">
                        <td class=\"\" style=\"width:220px\">
                            ...
                            <input type=\"text\" name=\"product_name\" onkeypress=\"delivery_note_productList('" . $product_details->product_id . "');\" class=\"form-control productSelection \" value='" . $product_details->product_name . "- (" . $product_details->product_model . ")" . "' placeholder='" . display('product_name') . "' required=\"\"  tabindex=\"\" readonly>
                       ...
                       $sl++; }
                           ...
                           $tr.="<input type=\"hidden\" id=\"total_discount_" . $product_details->product_id . "\" />
                            ...
                            <a style=\"text-align: right;\" class=\"btn delete_btn\" href=\"#\"  onclick=\"deleteRow(this)\">" . '<i class="fal fa-trash-alt"></i>' . "</a>
                             <a style=\"text-align: right;\" class=\"btn details_btn\" href=\"#\"  onclick=\"detailsmodal('".$product_details->product_name."','".$product_details->total_product."','".$product_details->product_model."','".$product_details->unit."','".$product_details->price."','".$product_details->image."')\">" . '<i class="fal fa-eye"></i>' . "</a>
                        </td>
                    </tr>";
            echo $tr;
        } else {
        ...
        //Insert pos delivery_note
    public function insert_pos_delivery_note() {
        $product_id      = $this->input->post('product_id',TRUE);
        $product_details = $this->delivery_note_model->pos_delivery_note_setup($product_id);
        $taxfield = $this->db->select('tax_name,default_value')
                ->from('tax_settings')
        ...
        if (empty($serialdata)) {
            ...
            $tr .= "<tr id=\"row_" . $product_details->product_id . "\">
                        <td class=\"\" style=\"width:220px\">
                            ...
                            <input type=\"text\" name=\"product_name\" onkeypress=\"delivery_note_productList('" . $product_details->product_id . "');\" class=\"form-control productSelection \" value='" . $product_details->product_name . "- (" . $product_details->product_model . ")" . "' placeholder='" . display('product_name') . "' required=\"\" id=\"product_name_" . $product_details->product_id . "\" tabindex=\"\" readonly>
    ...
    public function delivery_note_inserted_data_manual(){
        $data['title']      = display('delivery_note_print');
        $delivery_note_id         = $this->input->post('delivery_note_id',TRUE);
        $delivery_note_detail     = $this->delivery_note_model->retrieve_delivery_note_html_data($delivery_note_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
        ...
        $data['module']     = "delivery_note";
        $data['page']       = "delivery_note_html_manual"; 
        echo modules::run('template/layout', $data);
    }
   /*delivery_note no generator*/
      public function number_generator() {
        $this->db->select_max('delivery_note', 'delivery_note_no');
        $query      = $this->db->get('delivery_note');
        $result     = $query->result_array();
        $delivery_note_no = $result[0]['delivery_note_no'];
        if ($delivery_note_no != '') {
            $delivery_note_no = $delivery_note_no + 1;
        } else {
            $delivery_note_no = 1000;
        }
 ...
 public function bdtask_customer_autocomplete(){
        ...
        $customer_info  = $this->delivery_note_model->customer_search($customer_id);
     ...
     /*product autocomple search*/
        ...
        $product_name   = $this->input->post('product_name',TRUE);
        $product_info   = $this->delivery_note_model->autocompletproductdata($product_name);
     ...
     /*after selecting product retrieve product info*/
        ...
        $product_id   = $this->input->post('product_id',TRUE);
        $product_info = $this->delivery_note_model->get_total_product_invoic($product_id);
   ...
   $createdate = date('Y-m-d H:i:s');
    ...
    $customer_coa = [
         ...
         echo json_encode($data);
            ...
            public function bdtask_delivery_note_details_directprint($delivery_note_id = null){
         $delivery_note_detail     = $this->delivery_note_model->retrieve_delivery_note_html_data($delivery_note_id);
        $taxfield = $this->db->select('*')
                ->from('tax_settings')
                ->where('is_show',1)
